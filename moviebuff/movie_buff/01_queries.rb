def it_was_ok
  ...
  movies = Movie.where(score: 2..3).select(:id, :title, :score)
end
...
def harrison_ford
  ...
  Actor.select('movies.id, title').joins(:movies)
...
def biggest_cast
  ...
  movie_id = Movie.select("movies.id, movies.title").joins(:castings)
    .group('movies.id').order('COUNT(*) desc').limit(3)
...
def directed_by_one_of(them)
  ...
  Movie.where('actors.name IN (?)', them).joins(:director).select(:id, :title)
